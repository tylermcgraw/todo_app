...
const requiresAuthentication = (req, res, next) => {
  if (!res.locals.signedIn) {
    console.log("response:" + res.locals.signedIn);
    console.log("request:" + req.session.signedIn);
...
app.post("/users/signin",
  catchError(async (req, res) => {
    ...
    let validUser = await res.locals.store.isValidUser(username, password);
    if (validUser) {
...
app.post("/lists/:todoListId/todos/:todoId/toggle",
  ...
  catchError(async (req, res) => {
    ...
    let resultBoth = await Promise.all([toggled, todo]);

    if (!resultBoth[0]) throw new Error("Not found.");

    
    if (resultBoth[1].done) {
      req.flash("success", `"${resultBoth[1].title}" marked NOT done.`);
    } else {
      req.flash("success", `"${resultBoth[1].title}" marked as done!`);
    }
